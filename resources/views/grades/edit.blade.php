                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('grades.update', $grade) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="libelle" class="block mt-1 w-full" type="text" name="libelle" :value="old('libelle', $grade->libelle)" required autofocus autocomplete="libelle" />
                            <x-input-error :messages="$errors->get('libelle')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="salaire_base" :value="__('Salaire Base')" />
                            <x-text-input id="salaire_base" class="block mt-1 w-full" type="number" step="0.01" name="salaire_base" :value="old('salaire_base', $grade->salaire_base)" autocomplete="salaire_base" />
                            <x-input-error :messages="$errors->get('salaire_base')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="description" :value="__('Description')" />
                            <textarea id="description" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" name="description" rows="4">{{ old('description', $grade->description) }}</textarea>
