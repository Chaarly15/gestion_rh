        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('type-conges.update', $typeConge) }}">
                        @csrf
                        @method('PUT')

                        <div class="mb-4">
                            <x-input-label for="libelle" :value="__('Libellé')" />
                            <x-text-input id="libelle" class="block mt-1 w-full" type="text" name="libelle" :value="old('libelle', $typeConge->libelle)" required autofocus autocomplete="libelle" />
                            <x-input-error :messages="$errors->get('libelle')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="jours_max" :value="__('Nombre de jours maximum')" />
                            <x-text-input id="jours_max" class="block mt-1 w-full" type="number" name="jours_max" :value="old('jours_max', $typeConge->jours_max)" min="1" max="365" autocomplete="jours_max" />
